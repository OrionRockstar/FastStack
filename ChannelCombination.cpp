...
ChannelCombinationDialog::ChannelCombinationDialog(QWidget* parent) : ProcessDialog("ChannelCombination", { 570, 180 }, FastStack::recast(parent)->workspace(), false, false) {
	...
	addGreenInputs();
	addBlueInputs();
	connect(reinterpret_cast<const Workspace*>(m_workspace), &Workspace::sendClose, this, &CCD::onWindowClose);
	connect(reinterpret_cast<const Workspace*>(m_workspace), &Workspace::sendOpen, this, &CCD::onWindowOpen);
...
void ChannelCombinationDialog::addColorSpaceBG() {
	...
	rb->setChecked(true);
	m_colorspace_bg->addButton(rb, ColorSpace::rgb);
	gl->addWidget(rb, 0, 0);

	rb = new RadioButton("HSV", this);
	m_colorspace_bg->addButton(rb, ColorSpace::hsv);
	gl->addWidget(rb, 1, 0);

	rb = new RadioButton("HSI", this);
	m_colorspace_bg->addButton(rb, ColorSpace::hsi);
	gl->addWidget(rb, 2, 0);

	rb = new RadioButton("CIE XYZ", this);
	m_colorspace_bg->addButton(rb, ColorSpace::ciexyz);
	gl->addWidget(rb, 0, 1);

	rb = new RadioButton("CIE L*a*b*", this);
	m_colorspace_bg->addButton(rb, ColorSpace::cielab);
	gl->addWidget(rb, 1, 1);

	rb = new RadioButton("CIE L*c*h*", this);
	m_colorspace_bg->addButton(rb, ColorSpace::cielch);
	gl->addWidget(rb, 2, 1);
...
void ChannelCombinationDialog::Apply() {
	...
	if (status) {
		...
		ImageWindow32* iw = new ImageWindow32(rgb, "RGB Image", m_workspace);
	}
