    ...
    for (auto star : sv) {
        ...
        for (int j = yc - r; j <= yc + r; ++j)
            for (int i = xc - r; i <= xc + r; ++i)
                if (mask.isInBounds(i, j) && Distance(star.xc, star.yc, i, j) <= star.radius)
...
StarMaskDialog::StarMaskDialog(QWidget* parent) : ProcessDialog("StarMask", QSize(500,280), FastStack::recast(parent)->workspace(), false, true, false) {
	...
	connect(this, &ProcessDialog::processDropped, this, &StarMaskDialog::apply);
	ConnectToolbar(this, &ProcessDialog::CreateDragInstance, &StarMaskDialog::apply, &StarMaskDialog::showPreview, &StarMaskDialog::resetDialog);
