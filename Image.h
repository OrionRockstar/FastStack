...
public:
	...
	/*Image& operator=(const Image& other)noexcept {
		...
		if (this != &other) {
			...
			data = std::make_unique<T[]>(m_total_pixel_count);
	...
	template<typename P>
	bool matches(const Image<P>& other)const {
		return (m_rows == other.rows() && m_cols == other.cols() && m_channels == other.channels() && m_type == other.type());
	}
	...
	bool isSameShape(const Image<T>& other)const {
		if (m_rows != other.rows())
			return false;
		if (m_cols != other.cols())
			return false;
		if (m_channels != other.channels())
			return false;
		return true;
	}

	/*class ColorIterator {
		Iterator m_red = begin(0);
		Iterator m_green = begin(1);
		Iterator m_blue = begin(2);
	public:

		ColorIterator(Iterator r, Iterator g, Iterator b) : m_red(r), m_green(g), m_blue(b){}
		...
		bool operator==(ColorIterator other) {
		...
		template<typename P>
		Color<P> color() {
			P r = Pixel<P>::toType(*m_red);
			P g = Pixel<P>::toType(*m_green);
			P b = Pixel<P>::toType(*m_blue);

			return { r,g,b };
		}

		template<typename P>
		void setColor(const Color<P> color) {
			*m_red = Pixel<T>::toType(color.red());
			*m_green = Pixel<T>::toType(color.green());
			*m_blue = Pixel<T>::toType(color.blue());
		}
	};

	ColorIterator colorBegin() {
		return { begin(0), begin(1), begin(2) };
	}

	ColorIterator colorEnd() {
...
#pragma omp parallel for num_threads(2)
		for (int el = 0; el < m_pixel_count; ++el) {
			// << " " << m_pixel_count << " " << m_total_pixel_count << "\n";
...
#pragma omp parallel for num_threads(2)
		for (int el = 0; el < m_pixel_count; ++el) {
			double H, S, I;
			ColorSpace::RGBtoHSI(color<double>(el), H, S, I);
			setColor<>(el, Color<double>(H, S, I));
		}
	}
...
#pragma omp parallel for num_threads(2)
		for (int el = 0; el < m_pixel_count; ++el) {
			auto c = color<double>(el);
			setColor<>(el, ColorSpace::HSItoRGB(c.red(), c.green(), c.blue()));
		}
	}
	...
	void copyTo(Image<T>& dst)const {
		...
		memcpy(dst.data.get(), data.get(), m_total_pixel_count * sizeof(T));
	}

	void copyFrom(const Image<T>& src){
		if (!this->isSameShape(src))
			*this = Image<T>(m_rows, m_cols, m_channels);
		memcpy(data.get(), src.data.get(), m_total_pixel_count * sizeof(T));
	}
...
private:
	...
	/*bool isClipped(uint8_t pixel)const { return (pixel == Pixel<T>::min() || pixel == Pixel<T>::max()); }

	bool isClipped(uint16_t pixel)const { return (pixel == Pixel<T>::min() || pixel == Pixel<T>::max()); }

	bool isClipped(float pixel)const {

		if (fabsf(Pixel<T>::max() - pixel) < std::numeric_limits<float>::epsilon())
			return true;

		if (fabsf(Pixel<T>::min() - pixel) < std::numeric_limits<float>::epsilon())
			return true;
