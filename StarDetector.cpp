...
void StarDetector::addNewStar(int x, int y, int r, StarVector& star_vector) {
    ...
    for (struct Star& star : star_vector) {
        if (Distance(x, y, star.xc, star.yc) <= Max<int>(r, star.radius)) {
            //star.xc = .5 * (star.xc + x);
            //star.yc = .5 * (star.yc + y);
            star.radius = Max<int>(star.radius, r); //(star.rad + (r * ix2min_radius))
            return;
        }
    }
        ...
        for (int i = 0; i < svv[el].size(); ++i) {
            ...
            for (auto& s : starvector) {
                if (Distance(svv[el][i].xc, svv[el][i].yc, s.xc, s.yc) <= s.radius) {
                    s.radius = Max<int>(s.radius,svv[el][i].radius * mr[el] + 0.5);
                    new_star = false;
                    break;
                }
            }
...
template<typename T>
void StarDetector::aperturePhotometry_WCG(const Image<T>& img, StarVector& star_vector) {
    ...
    for (auto star = star_vector.begin(); star != star_vector.end();) {
        ...
        T t = b + 2 * AvgDev(local_background, b);
        ...
        for (double y = yc - rad, row = 0; y <= yc + rad; ++y) {
            for (double x = xc - rad; x <= xc + rad; ++x) {
       ...
       if (pix_count < 5) {
        ...
        star->luminance = -2.5 * log10(flux);
        star++;
    }
}
template void StarDetector::aperturePhotometry_WCG(const Image8&, StarVector&);
template void StarDetector::aperturePhotometry_WCG(const Image16&, StarVector&);
template void StarDetector::aperturePhotometry_WCG(const Image32&, StarVector&);

template<typename T>
bool StarDetector::gaussianFit(const Image<T>& img, const Star& star, PSF& psf, T b, T t) {

    float logA = logf(img(star.xc, star.yc));
    double flux = 0;
