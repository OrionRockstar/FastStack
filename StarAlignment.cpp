...
void StarAlignment::apply() {
	...
	auto stardetection = [this](Image8* img) {
		...
		case ImageType::USHORT:
			stars = m_sd.applyStarDetection(*reinterpret_cast<Image16*>(img));
			break;
		case ImageType::FLOAT:
			stars = m_sd.applyStarDetection(*reinterpret_cast<Image32*>(img));
			break;
		default:
...
StarAlignmentDialog::StarAlignmentDialog(QWidget* parent) : ProcessDialog("StarAlignment", QSize(540, 490), FastStack::recast(parent)->workspace(), false, false) {
	...
	ConnectToolbar(this, &ProcessDialog::CreateDragInstance, &StarAlignmentDialog::apply, &StarAlignmentDialog::showPreview, &StarAlignmentDialog::resetDialog);
