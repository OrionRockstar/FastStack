...
#pragma omp parallel for num_threads(2)
	for (int el = 0; el < img.pxCount(); ++el) {
		...
		img[el] = Pixel<T>::toType(Clip((r != 0) ? asinh(r * beta) / asinhb : beta / asinhb));
	}
}
...
void ASinhStretch::ApplyRGB(Image<T>& img) {
	...
	float max = 0;
	for (int i = 0; i < 3; ++i)
		max = Max(max, Pixel<float>::toType(img.computeMax(i)));
...
#pragma omp parallel for num_threads(2)
	for (int y = 0; y < img.rows(); ++y) {
		for (int x = 0; x < img.cols(); ++x) {
			...
			color.setRed(Clip(rescale(color.red()) * k));
			color.setGreen(Clip(rescale(color.green()) * k));
			color.setBlue(Clip(rescale(color.blue()) * k));
...
ASinhStretchDialog::ASinhStretchDialog(QWidget* parent): ProcessDialog("ASinhStretch",QSize(500,170),FastStack::recast(parent)->workspace()) {
	...
	connect(m_bp_comp, &QPushButton::pressed, this, [this]() { computeBlackpoint(); ApplytoPreview(); });
	...
	connect(m_rgb_cb, &QCheckBox::clicked, this, [this](bool v) {
		m_ash.setsRGB(v);
		ApplytoPreview(); });
...
void ASinhStretchDialog::showPreview() {
	...
	if (m_preview != nullptr) {
		...
		connect(previewRecast(m_preview)->windowSignals(), &WindowSignals::windowClosed, [this]() {m_bp_comp->setEnabled(false); });
	}
