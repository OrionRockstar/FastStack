...
private:
	...
	float interpolatePixel(const std::array<float, 4>& pixrow, const std::array<float, 4>& vec) {
	...
	float interpolatePixel(const std::array<float, 6>& pixrow, const std::array<float, 6>& vec) {
	...
	float nearestNeighbor(const Image<T>& img, const DoubleImagePoint& p) {
		...
		return img.at(p.x() + 0.5, p.y() + 0.5, p.channel());
	}
	...
	float Bilinear(const Image<T>& img, const DoubleImagePoint& p) {
		...
		if (!isBetween(x_f, 0, img.cols() - 1) || !isBetween(y_f, 0, img.rows() - 1)) {
			x_f += dx + .5;
			y_f += dy + .5;
			return img.at(x_f, y_f, p.channel());
		}
	...
	template <typename T>
	float bicubicSpline(const Image<T>& img, const DoubleImagePoint& p) {
		...
		if (!isBetween(x_f, 0, img.cols() - 2) || !isBetween(y_f, 0, img.rows() - 2)) {
			x_f += dx + .5;
			y_f += dy + .5;
			return img.at(x_f, y_f, p.channel());
		}
		...
		s.ry()--;
		resxv[0] = interpolatePixel(getKernelRow(img, s), vecx);
		s.ry()++;
		resxv[1] = interpolatePixel(getKernelRow(img, s), vecx);
		s.ry()++;
		resxv[2] = interpolatePixel(getKernelRow(img, s), vecx);
		s.ry()++;
		resxv[3] = interpolatePixel(getKernelRow(img, s), vecx);

		return interpolatePixel(resxv, vecy);
	}

	template <typename T>
	float bicubicBSpline(const Image<T>& img, const DoubleImagePoint& p) {
	...
	template <typename T>
	float cubicBSpline(const Image<T>& img, const DoubleImagePoint& p) {
	...
	template <typename T>
	float catmullRom(const Image<T>& img, const DoubleImagePoint& p) {
	...
	float Lanczos3(const Image<T>& img, const DoubleImagePoint& p) {
		...
		if (!isBetween(x_f, 1, img.cols() - 3) || !isBetween(y_f, 1, img.rows() - 3)) {
			x_f += dx + .5;
			y_f += dy + .5;
			return img.at(x_f, y_f, p.channel());
		}
		...
		ImagePoint s = { x_f,y_f,p.channel() };
		s.ry() -= 2;
		resxv[0] = interpolatePixel(getKernelRow6(img, s), vecx);
		s.ry()++;
		resxv[1] = interpolatePixel(getKernelRow6(img, s), vecx);
		s.ry()++;
		resxv[2] = interpolatePixel(getKernelRow6(img, s), vecx);
		s.ry()++;
		resxv[3] = interpolatePixel(getKernelRow6(img, s), vecx);
		s.ry()++;
		resxv[4] = interpolatePixel(getKernelRow6(img, s), vecx);	
		s.ry()++;
		resxv[5] = interpolatePixel(getKernelRow6(img, s), vecx);

		return interpolatePixel(resxv, vecy) / getWeight(vecx, vecy);
	}
...
public:
	...
	T interpolatePixel(const Image<T>& img, const DoubleImagePoint& p) {
		...
		float min = Pixel<T>::min();
		float max = Pixel<T>::max();
		...
		case bicubic_spline:
			return math::clip(bicubicSpline(img, p), min, max);

		case bicubic_b_spline:
			return math::clip(bicubicBSpline(img, p), min, max);

		case cubic_b_spline:
			return math::clip(cubicBSpline(img, p), min, max);

		case catmull_rom:
			return math::clip(catmullRom(img, p), min, max);

		case lanczos3:
			return math::clip(Lanczos3(img, p), min, max);
