...
HistogramDialog::HistogramDialog(const QString& img_name, const Image<T>& img, QWidget* parent) : QDialog(parent) {
	...
	//this->setWindowIcon(QIcon("./Icons//fast_stack_icon_fs2.png"));
	this->resize(630, 380);
	this->setMinimumSize(600, 280);


	this->setFocus();
	this->setAttribute(Qt::WA_DeleteOnClose);

	//Titlebar* tb = new Titlebar(QIcon("./Icons//fast_stack_icon_fs2.png"),"Histoooooooooooooooooo", *this);

	m_hist_view = new HistogramView({ 600,300 }, this);
	m_hist_view->drawHistogramScene(img);

	m_clip_cb = new CheckBox("Clip Shadows && Highlights", this);
	m_clip_cb->move(15, 15);
	m_clip_cb->setChecked(true);
	connect(m_clip_cb, &QCheckBox::clicked, this, [this](bool v) { m_hist_view->clipHistogram(v); });

	m_hist_data = new QLabel(this);
	m_hist_data->move(250, 18);
...
void HistogramDialog::resizeEvent(QResizeEvent* e) {
	...
	m_hist_view->setGeometry(15, 50, this->width() - 30, this->height() - 65);
}
