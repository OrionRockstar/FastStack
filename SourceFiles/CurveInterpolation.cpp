...
void GradientImage::setGradient(const std::vector<XRGB>& pts, float opacity) {
	uint8_t a = math::clip(opacity) * 255;
	...
	for (int y = 0; y < m_src.height(); ++y) {
		...
		for (int x = 0; x < m_src.width(); ++x) {
			...
			m_src.scanLine(y)[4 * x + 0] = 255 * math::clip(computePolynomial(m_bcoef, p, m_poly));
			m_src.scanLine(y)[4 * x + 1] = 255 * math::clip(computePolynomial(m_gcoef, p, m_poly));
			m_src.scanLine(y)[4 * x + 2] = 255 * math::clip(computePolynomial(m_rcoef, p, m_poly));
...
void GradientImage::setGradient(const std::vector<XRGB>& r, const std::vector<XRGB>& g, const std::vector<XRGB>& b, float opacity) {
	...
	for (int y = 0; y < m_src.height(); ++y) {
		...
		for (int x = 0; x < m_src.width(); ++x) {
			...
			m_src.scanLine(y)[4 * x + 0] = 255 * math::clip(computePolynomial(m_rcoef, p, m_poly));
			m_src.scanLine(y)[4 * x + 1] = 255 * math::clip(computePolynomial(m_gcoef, p, m_poly));
			m_src.scanLine(y)[4 * x + 2] = 255 * math::clip(computePolynomial(m_bcoef, p, m_poly));
			m_src.scanLine(y)[4 * x + 3] = a;
...
void GradientImage::setGradientRGB(float opacity) {
	...
	for (int y = 0; y < m_src.height(); ++y) {
		...
		for (int x = 0; x < m_src.width(); ++x) {
			...
			m_src.scanLine(y)[4 * x + 0] = 255 * math::clip(computePolynomial(m_gcoef, p - b_off, m_poly));
			m_src.scanLine(y)[4 * x + 1] = 255 * math::clip(computePolynomial(m_gcoef, p - g_off, m_poly));
			m_src.scanLine(y)[4 * x + 2] = 255 * math::clip(computePolynomial(m_rcoef, p + r_off, m_poly));
...
void GradientImage::cutCorners(Corner_t corner_flags) {
	...
	int min = math::min(m_src.width(), m_src.height());
