...
void CTD::apply() {
	...
	case ImageType::UBYTE:
		return currentImageWindow()->applyToSource(m_ct, &CurveTransform::apply);
	
	case ImageType::USHORT: 
		return currentImageWindow<uint16_t>()->applyToSource(m_ct, &CurveTransform::apply);
	
	case ImageType::FLOAT: 
		return currentImageWindow<float>()->applyToSource(m_ct, &CurveTransform::apply);
	}
}
...
void CTD::applyPreview() {
	...
	switch (preview()->type()) {
	case ImageType::UBYTE: 
		return preview()->updatePreview(m_ct, &CurveTransform::apply);
	
	case ImageType::USHORT: 
		return preview<uint16_t>()->updatePreview(m_ct, &CurveTransform::apply);
	
	case ImageType::FLOAT: 
		return preview<float>()->updatePreview(m_ct, &CurveTransform::apply);	
	}
}