...
GaussianFilterDialog::GaussianFilterDialog(QWidget* parent) : ProcessDialog("GaussianFilter", QSize(405, 105), FastStack::recast(parent)->workspace()) {
	...
	connectZoomWindow(this, &GaussianFilterDialog::applytoPreview);
...
void GaussianFilterDialog::addSigmaInputs() {
	...
	m_sigma_le = new DoubleLineEdit(new DoubleValidator(0.0, 20.0, 2), this);
...
void GaussianFilterDialog::applytoPreview() {
	...
	if (iwptr->parentWindow()->zoomWindow())
		m_gf.setSigma(m_sigma_le->text().toFloat() * iwptr->zoomWindowScaleFactor());
	else
