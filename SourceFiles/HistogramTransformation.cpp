...
void HistogramSlider::drawHandle(const QRect& rect, const QStyleOptionSlider& opt, QPainter& p) {
	...
	QRect r = rect;
	//r.translate((opt.sliderPosition - r.x()) - 1, 0);
...
HTD::HistogramTransformationDialog(QWidget* parent): ProcessDialog("HistogramTransformation", QSize(422, 400), FastStack::recast(parent)->workspace()) {
	...
	connectZoomWindow(this, &HTD::applytoPreview);
...
void HTD::applytoPreview() {
	...
	switch (iwptr->type()) {
	case ImageType::UBYTE: {
		return iwptr->updatePreview(m_ht, &HT::apply);
	}
	case ImageType::USHORT: {
